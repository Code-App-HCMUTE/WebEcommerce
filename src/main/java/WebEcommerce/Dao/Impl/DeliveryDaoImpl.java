...
public class DeliveryDaoImpl extends DBConnection implements DeliveryDao {
    ...
    public void edit(DeliveryModel delivery) {
        String sql = "UPDATE  delivery SET  name=?, description=?, price=?, isDeleted=?, updatedAt=?  WHERE _id = ?";
        try {
            ...
            ps.setDate(5, (Date) delivery.getUpdatedAt());
            ps.setInt(6, delivery.get_id());
            ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void delete(int id) {
        String sql = "DELETE FROM delivery WHERE _id = ?";
        try {
            Connection con = super.getConnection();
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
